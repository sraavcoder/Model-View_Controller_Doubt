...
def get_prediction(image):
    ...
    img_bw_resized_inv = PIL.ImageOps.invert(img_bw_resized)
    pixel_filter = 20
    min_pixel = np.percentile(img_bw_resized_inv, pixel_filter)
    image_bw_resized_inverted_scaled = np.clip(
        img_bw_resized_inv-min_pixel, 0, 255)
    max_pixel = np.max(img_bw_resized_inv)
